...
func (f *Frame) boxalign(cb0 int64, b0 int, pt0, pt1 p26) (int64, int, p26, p26) {
	type Pts [2]p26
	f.pts = f.pts[:0]
...
// from pt0 to pt1
func (f *Frame) boxpush(p0 int64, b0, b1 int, pt0, pt1, ppt1 p26) {
	...
	h := f.dy()
	if b0 == f.Nbox {
		f.Nlines = ((pt1.Y - f.r.Min.Y) / h).Ceil()
		if pt1.X > f.r.Min.X {
			f.Nlines++
		}
		return
	...
	if r := f.r; pt1.Y < r.Max.Y {
		...
		if qt1 < f.r.Max.Y {
			f.draw26(f.b, r, f.b, p26{f.r.Min.X, qt0}, f.op)
		}
		...
		r.Max.Y = qt1
		f.draw26(f.b, r, f.b, pt0, f.op)
	}
}

func (f *Frame) bitblt(cb0 int64, b0 int, pt0, pt1, opt0 p26) (res r26) {
	h := f.dy()
	y := i26(0)
	if pt1.Y == f.r.Max.Y {
		y = pt1.Y
	...
	for ; x >= 0; x-- {
		b := &run[x]
		br := r26{p26{0, 0}, p26{b.Width, h}}
		pt := f.pts[x]
		if b.Nrune > 0 {
			f.draw26(f.b, br.Add(pt[1]), f.b, pt[0], f.op)
			...
			if x == 0 && pt[1].Y > pt0.Y {
				...
				r := br.Add(opt0)
				r.Max.X = f.r.Max.X
				f.draw26(f.b, r, back, r.Min, f.op)
